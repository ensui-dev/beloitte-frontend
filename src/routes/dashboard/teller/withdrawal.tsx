...
import { CheckCircle2, Loader2, ArrowUpFromLine, AlertCircle, Search } from "lucide-react";
import { getAccountDisplayName } from "@/lib/data/types";
import type { Transaction, WithdrawRequest, BankAccount } from "@/lib/data/types";
...
export function TellerProcessWithdrawal(): React.ReactElement {
  ...
  const mutation = useMutation<Transaction, Error, WithdrawRequest>({
    ...
    onSuccess: () => {
      toast.success("Withdrawal processed", {
        description: `${formatCurrency(Number(amountStr), config!.currency)} withdrawn successfully.`,
      });
          ...
          <CardContent className="py-12">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold">Withdrawal Processed</h3>
                <p className="mt-1 text-sm text-muted-foreground">
                  {formatCurrency(Number(amountStr), config.currency)} withdrawn from{" "}
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="mx-auto max-w-lg border-white/[0.06] bg-white/[0.02] backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="text-lg">New Withdrawal</CardTitle>
          <CardDescription>
            Search for the customer&apos;s account, then enter the withdrawal amount.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between rounded-lg border border-primary/30 bg-primary/5 p-3">
                  ...
                  <div className="flex items-center gap-3">
                    {config && (
                      <span className="text-sm text-muted-foreground">
                        Balance: {formatCurrency(selectedAccount.balance, config.currency)}
                      </span>
                    )}
                    <Button
                      type="button"
