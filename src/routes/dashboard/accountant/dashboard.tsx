...
export function AccountantDashboard(): React.ReactElement {
  const { data: config } = useSiteConfig();
  const { data: stats, isLoading: statsLoading } = useAdminStats();
  const { data: volume, isLoading: volumeLoading } = useVolumeHistory();

  usePageTitle("Audit Dashboard", config?.bankName);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Read-only overview of bank operations and health.
        </p>
      </div>

      {/* Stats Grid (same as admin overview, read-only) */}
